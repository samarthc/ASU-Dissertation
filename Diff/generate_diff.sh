...
## TODO: make diffing bibliography data/style changes possible.
...
mv "$workdir"/git-latexdiff.$(cat "$workdir"/tmpsuffix)/* "$workdir" || {echo "Something went wrong moving tmp files -- exiting early" >&2; exit 1}

echo "Removing git-latexdiff.$(cat "$workdir"/tmpsuffix)..."
...
echo "Extracting only changed pages into $workdir/dissertation-onlychanges.pdf"
echo "..."

pdftk "$workdir"/dissertation.pdf cat $(perl -lne 'if (m/\\zref\@newlabel{DIFchgb(\d*)}{.*\\abspage{(\d*)}}/ ) { $start{$1}=$2; print $2 } if (m/\\zref\@newlabel{DIFchge(\d*)}{.*\\abspage{(\d*)}}/) {if (defined($start{$1})) {for ($j=$start{$1}; $j<=$2; $j++) {print "$j";} } else {print "$2" } }' "$workdir/new/dissertation.aux" | uniq | tr \\n ' ') output "$workdir"/dissertation-onlychanges.pdf

echo "Done."
read -p "Remove generated diff with redundant pages? [y/n]: " -n 1 yn
case $yn in 
    [Yy])
        echo "Removing redundant diff...";
        rm "$workdir"/dissertation.pdf || {echo "Something wenet wrong trying to delete redundant diff" >&2; exit 1}
        ;;
    [Nn])
        echo "Keeping redundant diff.";;
    *)
        echo "Invalid selection. Keeping redundant diff.";;
esac
