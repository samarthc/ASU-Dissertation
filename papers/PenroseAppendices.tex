...
\end{aligned}
\end{equation}
One way to compute \eqref{Penrose:eqn:spinorspincoeffs} is using the associated Newman-Penrose frame. Introducing first the Newman-Penrose spin coefficient symbols of \cite{penrose1984spinors,Penrose:1986ca}, which give names to the individual components,
\begin{alignat}{5}
...
\end{alignat}
Although we will not generally need the expressions \eqref{Penrose:eqn:spincoeffs} and symbols \eqref{Penrose:eqn:spincoeffssymbols} in the main text, they are useful performing explicit calculations for the examples. Moreover, the expressions above quoted from \cite{penrose1984spinors,Penrose:1986ca} retain the possibility to work with an unnormalized dyad, while we always impose \eqref{Penrose:eqn:dyadnorm}. Therefore
\begin{equation}
...
%\end{equation}
%which is sometimes called the Sachs operation \cite{penrose1984spinors,BenTov:2017kyf}, acts on %the curved metric frame in the same way provided one also exchanges $M$ and $N$ as well as $p$,$u$ %and $q$, $v$ in a particular way. Explicitly, \eqref{Penrose:eqn:Sachsflat} implies
%\begin{equation}
