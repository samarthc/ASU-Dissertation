\chapter{The Penrose Limit of the Weyl Double Copy} \label{chap:Penrose}

The contents of this chapter first appeared in Classical and Quantum Gravity \cite{Chawla:2024mse}.

\textit{We embed the Penrose limit into the Weyl classical double copy. Thereby,
we provide a lift of the double copy properties of plane wave spacetimes into black
hole geometries and we open a novel avenue towards taking the classical double copy
beyond statements about algebraically special backgrounds. In particular, the Penrose
limit, viewed as the leading order Fermi coordinate expansion around a null geodesic,
complements approaches leveraging asymptotic flatness such as the asymptotic Weyl
double copy. Along the way, we show how our embedding of the Penrose limit within
the Weyl double copy naturally fixes the functional ambiguity in the double copy for
Petrov type N spacetimes. We also highlight the utility of a spinorial approach to the
Penrose limit. In particular, we use this spinorial approach to derive a simple analytical
expression for arbitrary Penrose limits of four-dimensional, vacuum type D spacetimes.}
...
\paragraph*{Conventions} We will generally follow the conventions of Penrose and Rindler \cite{Penrose:1985bww, Penrose:1986ca}. An important exception is the Riemann curvature which differs by a sign from that reference. See also Appendix \ref{Penrose:app:conventions}.
...
The requirement that the geodesic congruence is twist-free arises as an integrability condition for it to be
written as a coordinate vector field. On the other hand, any geodesic can locally be embedded in a twist-free
geodesic congruence. More precisely, given any reference geodesic $\gamma$, a coordinate system can be found in a
neighborhood of a portion of $\gamma$ free of conjugate points\footnote{Conjugate points (or focal points) are a
pair of points such that there exists a Jacobi vector field (or equivalently here: a non-trivial solution to the
geodesic deviation equation) vanishing at both points. Conjugate points are sometimes referred to as caustics but,
more commonly, caustics are the geometric locus of conjugate points for a family of geodesics
\cite{Hawking:1973uf}.} such that the metric takes the form \eqref{Penrose:eqn:adapted} with (the conjugate
point-free portion of) $\gamma$ corresponding to the geodesic at $X^i = V = 0$ \cite{Penrose:1972ui}.
...
the valence-2 Killing spinor $\chi_{AB}$ for the Petrov type D black hole while on the Petrov
type N plane wave it is fixed by the Killing spinor $\beta_A^{(\gamma)}$.} \label{Penrose:fig:schematic}
\end{figure}
...
%The importance of $\chi_{AB}$ to the Weyl double is particularly clear if the single copy and zeroth copy are interpreted as living on the curved spacetime. In this ``curved background'' double copy, one can simply appeal to an old observation (see for instance \cite{Penrose:1986ca}) that $\chi^{AB}$ acts as a spin-lowering operator, relating solutions to the massless free field equations of different spin, to argue that the Bianchi identities for \eqref{Penrose:eqn:dcD} imply the field equations for \eqref{Penrose:eqn:zcD} and \eqref{Penrose:eqn:scD}. In the terminology of \cite{Penrose:1985bww,Penrose:1986ca}, a massless free field equation is in general an equation of the form
...
%\end{equation}
%To show that the field equations for $S$ are satisfied on the curved background, which is not strictly of the form \eqref{Penrose:eqn:masslessfree}, note that \eqref{Penrose:eqn:masslessfree} is equivalent to \cite{Penrose:1985bww} (2.5.24)
...
&  \\ & \\
	...
	\end{tikzcd}
        \caption{The Weyl double copy and Penrose limit of general vacuum Petrov type D spacetimes, in  terms of
            the valence-2 Killing spinor $\chi_{AB}$ and the associated constant of motion $\tilde{K}/\tilde{M}^2$ of
...
formalism in which one works covariantly with respect to such rescalings \cite{Penrose:1985bww}. We will say a
