...
In the next section, we review the classical double copy and the spinor formalism needed for its Weyl formulation, as well as the  Kerr-NUT-(A)dS spacetimes and their properties. In Section \ref{sec:FiveDDoubleCopy}, we construct the five-dimensional single copy using the Weyl spinor, and show its equivalence to the multi-Kerr-Schild ansatz.  We additionally note that this field is aligned to the principal tensor of the spacetime.  In Section \ref{sec:GeneralDSingleCopy}, we use a multi-Kerr-Schild approach, showing that the single copy of any Kerr-NUT-(A)dS spacetime is an aligned field.  Finally, in Section \ref{sec:discussion}, we discuss our results and directions for future work.  
...
\eqref{KNAmetricInCanonicalCoordinatesGeneralD} is
%
...
A spacetime whose metric is in the Kerr-Schild form \eqref{Kerr-SchildMetric} has a simple proposal for its
single copy. The vacuum Einstein equations further guarantee that the single copy field is on-shell.
%
The Kerr-NUT-(A)dS metric \eqref{KNAmetricInCanonicalCoordinatesGeneralD} can be written in a similar way: a multi-Kerr-Schild
...
Myers-Perry latitude coordinates $\mu_{i}$ despite their usage of the same symbol. Their explicit form in terms of the orthonormal coframe \eqref{DarbouxCoFrame}
is
%
...
\end{equation}
%
Since the single copy and zeroth copy fields (usually, see footnote \ref{FlatVCurvedFootnote}) solve equations of motion on
...
In section \ref{sec:GeneralDSingleCopy}, we will use the multi-Kerr-Schild form to propose a single copy gauge
...
carried out. We will refer to any basis that satisfies \eqref{TypeDConditions} as a Weyl-aligned frame.
...
section \ref{sec:FiveDDoubleCopy}. The following is a summary of relevant spinor definitions from
...
\overline{m}^{\mu}, e_{0}^{\mu})$. Any two bases which satisfy \eqref{SpinorBasisDefn} and
\eqref{SpinorBasisNormalization} are related by an $SU(2)$ action on lowercase latin indices.
...
\eqref{TypeDConditions} hold; see Appendix \ref{SpinorConventions}. The irreps contained within
...
\end{multline}
%
In the Darboux basis \eqref{DarbouxCoFrame} the Weyl tensor is
%
...
\end{equation}
%
$ \hat{Q}$ is defined in terms of metric functions \eqref{KNAmetricFunctions} as\footnote{Our definition of
...
\end{equation}
%
The Weyl tensor \eqref{WeylInFormNotation5D}  for Kerr-NUT-(A)dS exhibits extra algebraic symmetries beyond those required of all Weyl curvature tensors. For instance, swapping
...
well-defined square root $f_{AB}$ in the sense of \eqref{WeylSpinorDoubleCopyRelation} exists, which allows for
...
section \ref{sec:ReviewSpinors5D} with a few further details in Appendix \ref{SpinorConventions}.

As mentioned in section \ref{sec:WeylSpinorLittleGroupObjects} and shown explicitly in Appendix
\ref{SpinorConventions}, for a type D spacetime in a Weyl-aligned frame (i.e. a frame that satisfies the type D
conditions \eqref{TypeDConditions}) the only nonzero little group object is $\Psi^{(2)}$
\eqref{WeylLittleGroupObjects}. Thus, in the $(k^{a}_{A}, n^{a}_{A})$ basis the Weyl spinor is 
%
...
The irreps \eqref{LittleGroup2Irreps} of $\Psi^{(2)}_{ABCD}$ are
\begin{equation} 
    \begin{split}
...
In terms of the
general expansion of $\psi^{(2)}$ and $\chi^{(2)}$ \eqref{IrrepExpansion}, the only nonzero components are $\psi^{(2)}_{2}$ and
...
\eqref{WeylSpinorDoubleCopyRelation}, which we fix%
%
...
\eqref{PrincipalTensorInDarbouxCoFrame}. That is, they have the expansion \cite{Krtous:2007xg}
%
...
\end{equation}
%
and plug it into the Weyl doubling relation \eqref{WeylDoublingRelation}. We find that the relation is
...
\footnote{\cite{Luna:2018dpt} claims a slightly different gauge
    ...
    coordinates \eqref{TransformAzimuthalAnglesFromCanonicalToMyersPerry}. Then, we can 
...
\eqref{MultiKerrSchildMetric} and propose the single copy gauge field
%
\begin{equation} \label{5D:singleCopyMultiKerrSchild} 
    \mathbf{A} = \sum_{k = 1}^{n} \frac{2Q_{k}x_{k}^{1-\varepsilon}}{U_{k}} \boldsymbol{\mu}^{k},
\end{equation}
%
where $ \boldsymbol{\mu}^{k}$ are the null geodesic $1$-forms \eqref{MuDefn} and $Q_{k}$ are constant parameters which satisfy
...
\eqref{singleCopyMultiKerrSchild},
...
$ D(D-2)\lambda/4$ for dimensions other than $D = 4$ or $D=6$.} of $2(D-3) \lambda$. As pointed out previously in section \ref{sec:FiveDDoubleCopy}, such an equation of
...
\eqref{WeylDoublingRelation}. However, plugging in the Weyl tensor components \eqref{WeylInFormNotation5D} and the
components of the aligned field from \eqref{AlignedFieldExpansion} and \eqref{AlignedFieldComponents}, there is no
scalar that satisfies \eqref{WeylDoublingRelation} for any value of aligned field parameters $Q_{k}$ given
...
\eqref{WeylDoublingRelation}. Since our proposed single copy field \eqref{singleCopyMultiKerrSchild} and zeroth
copy \eqref{zerothCopyMultiKerrSchild} are well-defined and satisfy the expected equations of motion, we thus
...
The authors thank Damien Easson, Andres Luna, Tucker Manton, and Silvia Nagy for helpful discussions. The authors are supported by the U.S. Department of Energy under grant number DE-SC0019470 and by the Heising-Simons Foundation “Observational Signatures of Quantum Gravity” collaboration grant 2021-2818.
