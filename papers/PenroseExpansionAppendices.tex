	\chapter{Penrose Expansion: Spinor conventions and Newman-Penrose formalism}\label{Expansion:app:NP}
	...
	\begin{aligned}
	U &= \int^{r(U)}_{r_{\textrm ref}} \frac{d\rho}{R'(\rho) -\frac{2M}{\rho} \left(\hat{E}+R'(\rho)\right)}   \, , \\ 
	\hat{t}(U,\hat{t}_0) &= \hat{t}_0 + \int^{r(U)}_{r_{\textrm ref}} d\rho \, \frac{\hat{E}+\frac{2M}{\rho} \left(\hat{E}+ R'(\rho)\right)}{R'(\rho) -\frac{2M}{\rho} \left(\hat{E}+R'(\rho)\right)} \, , \\
	\theta_0(V,\hat{t}_0) &= \frac{1}{L}\left(\hat{E}\hat{t}_0  - V\right) \, , \\
	\theta(U,V, \hat{t}_0) &= \theta_0(V,\hat{t}_0)+ L\int^{r(U)}_{r_{\textrm ref}} d\rho \, \frac{1}{\rho^2}\frac{1}{R'(\rho) -\frac{2M}{\rho} \left(\hat{E}+R'(\rho)\right)} \, ,\\
	\phi(U,V,\phi_0,\hat{t}_0) &= \phi_0  \, ,
	\end{aligned}
	\end{equation}
	where $r_{\textrm ref}$ is an arbitrary constant while $r(U) = r(U;\hat{E},L^2)$ is a radial solution to the geodesic equations. Explicitly, $r(U)$ satisfies
	\begin{equation}
	\frac{dr}{dU} = \left(1-\frac{2M}{r(U)}\right)R'(r(U)) - \frac{2M \hat{E}}{r(U)} \, , 
	\end{equation}
	with initial condition $r(0) = r_{\textrm ref}$ and where $R(r)$ satisfies
	...
	\begin{equation}
	\begin{aligned}
	T_{1}^{(1)}(U) &= \frac{a^{(0)}_{S,0,0,0}}{\sqrt{a^{(0)}_{\Psi_0,0,0,0} a^{(0)}_{S,0,0,0}}} \left(
        -\frac{da^{(1)}_{\Psi_1,0,0,0}}{dU}   -  a^{(1)}_{\Psi_1,0,0,0} \frac{d \log a^{(0)}_{S,0,0,0}}{2
    dU}+a^{(1)}_{\Psi_1,0,0,0} \frac{d\log C}{2 dU} \right. \\ & \left. +\frac{i \overline{m}_{1} }{2 \sqrt{C}}
a^{(1)}_{\Psi_0,0,0,1}  -\frac{i \overline{m}_{2}}{2 \sqrt{C}}
a^{(1)}_{\Psi_0,0,1,0}+a^{(1)}_{\Psi_1,0,0,0}\frac{d \log a^{(0)}_{\Psi_0,0,0,0}}{2 dU} \right. \\
                                                               & \left. +\frac{i}{\sqrt{C}} (\partial_{1}\overline{m}_{2}-\partial_{2}\overline{m}_{1})a^{(0)}_{\Psi_0,0,0,0} \right)  \, ,
	\end{aligned}
	\end{equation}
	and for $T_{2}^{(1)}(U)$
	\begin{equation}
