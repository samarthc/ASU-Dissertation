...
% Preamble
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand*{\pointsize}{11pt}          %_<Set the font size; make sure the size is correct
...
\usepackage{microtype}                  %_~Improves kerning and protrusion (optional);
...
%\usepackage{ifplatform}                 % Includes conditionals for testing OS that was used to compile. Requires
                                        % use of ``shell-escape'' option when compiling. Used to test if this was
                                        % compiled inside the template's provided Docker container (linux/Debian),
                                        % or on my local machine (MacOS). This is of course not robust enough for
                                        % general use - it fails in the case when the local system is running Linux

                                        % Why this exists: 
                                        % 1) sometimes I include additional packages from CTAN for
                                        %    testing, and I would rather not rebuild the Docker image each time. I
                                        %    test additional packages on my local machine, and if deemed necessary,
                                        %    add those packages to the latexmk Dockerfile and rebuild.
                                        % 2) to select the correct location of the relevant font file.

                                        % A robust way of checking it would maybe be to modify the Dockerfile to
                                        % set a certain environment variable, then write a LaTeX conditional to
                                        % check the existence/value of the environment variable? Or perhaps check
                                        % for the existence of a certain file that would only exist in the Docker
                                        % coontainer?

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Author-defined preamble
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{chapters/Preamble}
\input{papers/Preamble}             % Additional packages and settings for various chapters.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Title page, input
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\listadd{\titlelines}%
  {Explorations in the Classical Double Copy:}             %_<Enter the title of the dissertation
\listadd{\titlelines}%
{Higher Dimensions, Horizons and Penrose Limits}            % If you want to
                                        %   split the title across lines,
                                        %   use another \listadd command for the
                                        %   second line
\newcommand*\Author{Samarth Chawla}          %_<Enter your name; must match official transcript
\newcommand*{\documentname}%
  {Dissertation}                        %_<Enter the type of document (capitalized)
\newcommand*{\degreename}
  {Doctor of Philosophy}                %_<Enter the type of degree (capitalized)
\newcommand*\defdate{April 2026}        %_<Give month (written out fully) and year of
                                        %   the oral defense
\listadd{\committeechair}{Cynthia Keeler}   %_<Enter committee chair name; use \listadd for
%\listadd{\committeechair}{Another name}%   for additional names
\newcommand*{\chairlabel}{Chair}        %_<If you have co-chairs, replace this text with
                                        %   'Co-Chair'
\listadd{\committeemember}{Damien Easson} %_<Enter committee member names; use \listadd for
\listadd{\committeemember}{Maulik Parikh} %   additional names
\listadd{\committeemember}{Tanmay Vachaspati} %   additional names
\newcommand*{\gradmonth}{May}         %_<Enter the graduation date; month can only be:
                                        %  May, August, or December
\newcommand*{\gradyear}{2026}           %_<Enter the graduation year, e.g. 2014

\listadd{\keywords}{Classical Double Copy}          %_<Enter keywords; use \listadd for
\listadd{\keywords}{Black Hole Spacetimes}          %   additional names (up to 6)
\listadd{\keywords}{Black Hole Horizons}          %   additional names (up to 6)
\listadd{\keywords}{Penrose limit}          %   additional names (up to 6)
\listadd{\keywords}{Weyl Double Copy}          %   additional names (up to 6)
\listadd{\keywords}{Kerr-Schild Spacetimes}          %   additional names (up to 6)
...
\setlrmarginsandblock{1.25in}{1.25in}{*}%_~Set margins and calculate width of typeblock
...
\ifnum 0\ifxetex 1\fi\ifluatex 1\fi=0   % If pdftex
  \usepackage[utf8]{inputenc}           %   'utf8' should match the encoding of this file
                                        %
                                        %_~Set up your font in pdftex here
                                        %
\else 				        % If xetex or luatex
  ...
  %]
  %\iflinux
      %\setmainfont{Georgia}[                %_<Set the main font; make sure the font is correct
        %Path          = /usr/share/fonts/truetype/msttcorefonts/,%%   for the font size (See ASU Style Guide)
        %Extension     = .ttf,
        %UprightFont   = *,
        %ItalicFont    = *_Italic,
        %BoldFont      = *_Bold,
...
%
% UPDATE: As of August 2025, hyperref is no longer an issue
...
%\renewcommand{\cftsectionfont}%
  %{\normalfont\titlecap}%              % Intended to make section, subsection and 
%\renewcommand{\cftsubsectionfont}%     % subsubsection headings in titlecase, but
  %{\normalfont\titlecap}%              % does not work...
%\renewcommand{\cftsubsubsectionfont}%
  %{\normalfont\titlecap}


%\renewcommand\cftsectionaftersnumb{\titlecap} % Also did not work
...
\providecommand\totfig{0}                % Retrieve the total number of figures
\providecommand\tottab{0}                % Retrieve the total number of tables
...
\settoggle{useendnotes}{false}           %_<Set to 'true' if you want to use endnotes
...
% Bibliography
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\bibfilename}{%             %_<Enter the name of the *.bib file containing the
  papers/PapersCombined%                %   reference information for sources cited in
}%                                      %   the text. God help you if you're doing
                                        %   citations manually.

\newcommand{\bibheading}{Bibliography}    %_<Enter the heading for the references section:
...
\settoggle{usebiblatex}{true}           %_~Set to 'false' to use 'natbib' intead of
...
%                                       %   'biblatex'
  \usepackage[style=phys,%              % Style option for AIP/APS style
              %authordate,%
              backend=biber,%           % Recommend to use 'biber' instead of 'bibtex'
              pageranges=false,%        % APS style requires only first page of journal 
                                        % article, not the full range.
              biblabel=brackets,%       % APS style requires bracketed citations, not
                                        % the default superscript option
              %doi=only,%               % Avoid printing URLs [default false for AIP/APS]
              %isbn=false               % Don't print ISBN numbers [default false for 
                                        % AIP/APS]
              arxiv=abs,%               % Ensure arXiv links are to the abs page, not 
                                        % the pdf page.
  ]%              
  {biblatex}                            %_~Other possibilities include: 'biblatex',
                                        %   'biblatex-apa', and 'biblatex-mla'

  % Need to redefine the BibliographyDriver for @online entries, which is the default 
  % type for arXiv preprints. The default driver includes unnecessary information, like
  % the full url, url access date and publication status ("pre-published").
  % Bibliography driver based on the driver definition for @article, from phys.bbx, and
  % the definition for @online, from biblatex/standard.bbx
  ...
  \newcommand\RefList{}
  \newcommand\AddToRefList[1]{\xdef\RefList{\RefList,#1}}

  \AtEveryCitekey{\AddToRefList{\thefield{entrykey}}}
  ...
  \usepackage{natbib}%
  \newcommand{\natbibstyle}{%           %_~Enter the name of the *.bst file to use to
...
% Tables and figures
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\captiondelim{. }                       %_~Use period (.) after caption number instead of
...
\hypersetup{                            % Set pdf metadata
 ...
 %pdfproducer={},                       % Let 'pdfproducer' be filled automatically
  ...
  hidelinks,                            % Links highlighting turned off
...
% Copyright page
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\copyrightpageASU                       %_~If you don't want to have a copyright page,
...
%\chapter*{Dedication}                   %_~Dedication is optional
%\clearpage                             %_~If you don't wish to display the heading
...
\chapter*{Acknowledgments}              %_~Acknowledgments are optional
...
%\chapter{Preface}                      %_~PREFACE (optional, less than 10 pages)
%\input{preface}                        %_<Enter the name of the .tex file or omit this
                                        %   line and type in here.
...
\include{papers/5D}
\newrefsection
\include{papers/Horizons}
\newrefsection
\include{papers/Penrose}
\newrefsection
\include{papers/PenroseExpansion}
\endrefsection
...
\iftoggle{usebiblatex}{%                % Output the bibliography
  ...
  \nocite{\RefList}
  \printbibliography[heading=none,section=0]      % Using a 'biblatex' package; do not let
...
\input{papers/5DAppendices}
\input{papers/HorizonsAppendices}
\input{papers/PenroseAppendices}
\input{papers/PenroseExpansionAppendices}
\input{chapters/authorpermissions}
\iftoggle{sample}{%
...
%\biographicalsketch{%                  %_~Biographical Sketch is optional
%  \input{biography}%                   %_<Enter the name of the .tex file containing your
